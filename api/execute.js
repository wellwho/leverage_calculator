...
//              this rarely matches the plan's numbers exactly) is reported back as
//              `leftoverCapital` — informational only. It's intentionally never pushed
//              to the position as margin: this app assumes MEXC's own Auto-Margin
//              feature (enabled per-position in the MEXC app) will draw on the
//              account's available balance and top up margin before liquidation, so
//              there's nothing for this app to do with it beyond leaving it as
//              available balance. Optional: if omitted (e.g. an older frontend),
//              leftoverCapital comes back null.
//
...
module.exports = async (req, res) => {
  ...
  // plan's theoretical numbers — is reported back as informational only.
  // It intentionally stays as available futures balance rather than being
  // pushed into the position: this app assumes MEXC's own Auto-Margin
  // feature will draw on that available balance and top up the position
  // before liquidation, so there's nothing for this app to actively do here.
  ...
  res.status(200).json({ contractSize, results, leftoverCapital });
};
